...
# ==========================================
st.set_page_config(page_title="Dashboard Mystery Shopper Demo", page_icon="🛒", layout="wide")
...
def cargar_geojson_local():
    if os.path.exists("guatemala.geojson"):
        with open("guatemala.geojson", "r", encoding="utf-8") as f:
            geo = json.load(f)
            for feature in geo['features']:
                nom = feature.get('properties', {}).get('shapeName', '') or feature.get('properties', {}).get('name', '') or feature.get('properties', {}).get('NAME_1', '')
...
def cargar_datos_demo():
    if os.path.exists("Base_Simulada_Demo_CX.csv"): return pd.read_csv("Base_Simulada_Demo_CX.csv")
    return None

df = cargar_datos_demo()
geojson_gt = cargar_geojson_local()
if df is None: st.warning("⚠️ No se encontró 'Base_Simulada_Demo_CX.csv'."); st.stop()

df['Fecha'] = pd.to_datetime(df['Fecha'], format='%d/%m/%Y', errors='coerce')
df['Mes_Nombre'] = df['Fecha'].dt.month.map({1: 'Enero', 2: 'Febrero', 3: 'Marzo', 4: 'Abril', 5: 'Mayo', 6: 'Junio', 7: 'Julio', 8: 'Agosto', 9: 'Septiembre', 10: 'Octubre', 11: 'Noviembre', 12: 'Diciembre'})
...
k_journey = {'Estacionamiento': ['GUARDIA', 'ATENCIÓN', 'ATENCION'], 'Bienvenida': ['PERSONAL', 'AMABLE', 'BIENVENIDA', 'ATENCIÓN', 'ATENCION'], 'Sector 1': ['PERSONAL', 'ATENDIÓ', 'ATENDIO', 'AMABLE', 'CONOCIMIENTO'], 'Sector 2': ['PERSONAL', 'AMABLE', 'CONOCIMIENTO'], 'Pasillo secundario': ['PERSONAL', 'GUIARON'], 'Cobro': ['PERSONAL', 'SALUDÓ', 'SALUDO', 'AMABLE', 'OFRECIÓ', 'OFRECIO', 'DEJÓ', 'DEJO', 'REACCIONÓ', 'REACCIONO', 'AGRADECIÓ', 'AGRADECIO'], 'Empaque': ['PERSONAL', 'AMABLE', 'ÁGIL', 'AGIL'], 'Experiencia': ['PERSONAL', 'ASESORÍA', 'ASESORIA', 'ATENCIÓN', 'ATENCION']}
k_calidad = {'Estacionamiento': ['ESPACIOS', 'DISPONIBLES'], 'Bienvenida': ['ELEMENTOS', 'LIMPIOS', 'BASURA', 'DESPLAZABAN'], 'Pasillo central': ['LIMPIO', 'BASURA', 'EXHIBICION', 'EXHIBICIÓN', 'VACIA', 'VACÍA'], 'Sector 1': ['OLOR', 'LIMPIO', 'BASURA'], 'Sector 2': ['LIMPIO', 'BASURA'], 'Sector 3': ['EXHIBICION', 'EXHIBICIÓN', 'COMPLETA', 'LIMPIO', 'BASURA'], 'Pasillo secundario': ['LIMPIO', 'BASURA', 'CAJAS', 'OBSTACULIZA'], 'Cobro': ['PUNTO DE COBRO', 'DISPONIBLE'], 'Empaque': ['OPCIONES', 'SUFICIENTES'], 'Sanitarios': ['DISPONIBLE', 'SUMINISTROS', 'FUNCIONAN', 'LIMPIEZA', 'OLIA', 'OLÍA'], 'Experiencia': ['AMBIENTE', 'ILUMINACION', 'ILUMINACIÓN', 'DISEÑO', 'MUSICA', 'MÚSICA', 'AROMA']}
cols_preguntas = [c for c in df.columns if '[' in c and ']' in c]
preguntas_inversas = ['vacía', 'vacia', 'mal olor', 'obstaculiza'] 
...
mapa_columnas_limpias, preguntas_por_categoria, categorias_limpias_list = {}, {}, []
for col in cols_preguntas:
    ...
    if cat_clean not in categorias_limpias_list: categorias_limpias_list.append(cat_clean); preguntas_por_categoria[cat_clean] = []
...
for idx, row in df.iterrows():
    ...
    for col in cols_preguntas:
        ...
        if respuesta not in ['NAN', 'N/A', 'NO APLICA', '', 'NONE']:
            ...
            if es_positivo: puntos_ganados += 1; desempeno_puntos[cat_clean]['ganados'] += 1
            desempeno_puntos[cat_clean]['validos'] += 1
            pc_up, preg_up = col.split('[')[0].upper(), pregunta.upper()
...
with cm1:
    ...
    st.plotly_chart(fig_g, use_container_width=True, key="macro_gauge")
...
with cm2:
    ...
    if geojson_gt:
        ...
        for d in TODOS_DEPTOS_CLEAN:
            ...
            cols.append(color_semaforo(sc) if sc is not None else '#D3D3D3')
...
with c_top:
    ...
    ev_t = st.plotly_chart(fig_t, use_container_width=True, key="chart_top40", on_select="rerun")
with c_bot:
    ...
    ev_b = st.plotly_chart(fig_b, use_container_width=True, key="chart_bot40", on_select="rerun")
...
with ct1:
    st.subheader("Evolución Histórica (Global)")
    ...
    st.plotly_chart(fig_tr, use_container_width=True, key="chart_tendencia")
with ct2:
    ...
    ev_sec = st.plotly_chart(fig_s, use_container_width=True, key="chart_sector", on_select="rerun")
...
if st.session_state.tienda_seleccionada:
    ...
    st.markdown("<p style='color: #A0A0A0; font-size: 14px; margin-top:-10px;'><i>Histórico de la tienda seleccionada (independiente del filtro de mes).</i></p>", unsafe_allow_html=True)
    
    df_store = df[df['Nombre_Grafica'] == st.session_state.tienda_seleccionada].dropna(subset=['Score_Operativo']).sort_values('Fecha')
    if not df_store.empty:
        c_evo1, c_evo2 = st.columns([2.5, 1])
        df_store_trend = df_store.groupby('Mes_Año', sort=False)['Score_Operativo'].mean().reset_index()
        with c_evo1:
            fig_evo = px.line(df_store_trend, x='Mes_Año', y='Score_Operativo', markers=True)
            fig_evo.update_traces(line_color='#F2A900', marker=dict(size=10, color='#0A1B3F'))
            fig_evo.update_layout(height=250, xaxis_title="", yaxis_title="Score %", yaxis=dict(range=[0, 100]), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=0, t=10, b=0))
            st.plotly_chart(fig_evo, use_container_width=True, key="chart_evo_tienda")
            
        with c_evo2:
            avg_val = df_store['Score_Operativo'].mean(); max_val = df_store['Score_Operativo'].max(); min_val = df_store['Score_Operativo'].min()
            if len(df_store_trend) > 1:
                curr_val = df_store_trend.iloc[-1]['Score_Operativo']; prev_val = df_store_trend.iloc[-2]['Score_Operativo']; delta_val = curr_val - prev_val
                st.metric("Último Mes vs Anterior", f"{curr_val:.1f}%", f"{delta_val:.1f}%")
            else: st.metric("Último Mes", f"{df_store_trend.iloc[-1]['Score_Operativo']:.1f}%")
            st.metric("Pico Máximo", f"{max_val:.1f}%", delta=f"{(max_val - avg_val):.1f}% vs Promedio", delta_color="normal")
            st.metric("Punto Crítico", f"{min_val:.1f}%", delta=f"{(min_val - avg_val):.1f}% vs Promedio", delta_color="inverse")
    st.markdown("---")
...
if cols_cat:
    ...
    with cp2:
        ...
        if cat_sel:
            ...
            st.plotly_chart(fig_d, use_container_width=True, key="chart_drilldown")
...
def render_chevrons(keys_list, class_prefix):
    n = len(keys_list); html = '<div class="journey-container">'
...
if st.session_state.tienda_seleccionada:
    ...
    with c_alertas:
        ...
        else: st.info("No hay hallazgos graves registrados.")
    with c_comentarios:
        ...
        else: st.info("No hay comentarios registrados.")
